...
def test_followup_class():
    ...
    expected = [-6.6000834193414635, 0.36024705241286203, 0.04326409573404126, 
...
def test_followup_spline():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-6.264817962084417, 0.20125056343026881, 0.12568743032952776, 
...
def test_no_followup():
    ...
    s.fit()
    matrix = s.outcome_model[0]['outcome'].summary2().tables[1]["Coef."].to_list()
    expected = [-6.062350570326165, 0.17748844870984498, 0.11209431124681817,
